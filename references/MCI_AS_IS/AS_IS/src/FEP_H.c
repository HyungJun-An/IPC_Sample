...
int main(int argc, char* argv[]) 
{
    ...
    while(1)
    {
        ...
        { 
            printf("FEP_H Message Receiving Finished!\n");
            exit(0);
        }
...
int init(void )
{
    ...
    /* 수신 MessageQueue 연결         */
    /**********************************/
    mq_recv_key = msgget((key_t) 2345, IPC_CREAT|0666);
    if (mq_recv_key == -1)
    {
        printf("FEP_H Message Get Failed!\n");
        return -1;
    }
    ...
    /* 송신 MessageQueue 연결         */
    /**********************************/
    mq_send_key = msgget((key_t) 4567, IPC_CREAT|0666);
    if (mq_send_key == -1)
    {
        printf("FEP_H Message Get Failed!\n");
        return -1;
