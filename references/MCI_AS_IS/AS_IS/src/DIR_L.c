...
int main(int argc, char *argv[])
{
    ...
    if( argc != 2)
    {
        printf("DIR_L Use Error : processname IP Port 'Ex : DIR_B 127.0.0.1' ");
        exit(0);
    }
    rtn = init(argv);
    if(rtn == -1)
    {
        printf("DIR_L [ERROR] INIT \n"); 
    }
    ...
    while(1)
    {
        ...
        if( poll_result == -1)
        {
            printf("DIR_L poll error \n");
            break;
        }
        if( poll_result == 0) 
        {
            printf("DIR_L poll timeout \n");
            continue;        /* timeout */
        }
...
int init(char* argv[] )
{
    ...
    /* MessageQueue 연결              */
    /**********************************/
    mq_key = msgget((key_t) 7890, IPC_CREAT|0666);
    if (mq_key == -1) 
    {
        printf("DIR_L Message Get Failed!\n");
        exit(0);
    }
	...
	if (server_sock == -1)
	{
		printf("DIR_L socket create error!\n");
		return 0;
	}
